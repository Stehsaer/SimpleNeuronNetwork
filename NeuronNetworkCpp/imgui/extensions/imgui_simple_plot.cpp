...
namespace ImGui {
    namespace SimplePlot {
        ...
        ImVec2 Plus(ImVec2 left, ImVec2 right)
        {
            return ImVec2(left.x + right.x, left.y + right.y);
        }

        ImVec2 Minus(ImVec2 left, ImVec2 right)
        {
            return ImVec2(left.x - right.x, left.y - right.y);
        }
        ...
        PlotStatus Plot(const char* label, const PlotConfig& conf) {
            ...
            const ImRect frame_bb(
                window->DC.CursorPos,
                Plus(window->DC.CursorPos, conf.frame_size));
            const ImRect inner_bb(
                Plus(frame_bb.Min, style.FramePadding),
                Minus(frame_bb.Max, style.FramePadding));
                        ...
                        while (start < x_max) {
                            for (int i = 1; i < 10; ++i) {
                                ...
                                window->DrawList->AddLine(
                                    ImVec2(x0, y0),
                                    ImVec2(x0, y1),
                                    IM_COL32(100, 100, 100, (i > 1) ? 128 : 255));
                            }
                            start *= 10.f;
                        }
                        break;
                ...
                if (conf.grid_y.show) {
                    ...
                    for (int i = 0; i <= cnt; ++i) {
                        ...
                        window->DrawList->AddLine(
                            ImVec2(x0, y0),
                            ImVec2(x1, y0),
                            IM_COL32(200, 200, 200, (i % conf.grid_y.subticks) ? 128 : 255));
                    }
                }
